    ...
    public class GradientRadial_Tests
    {
        ...
        public void ReflectionalSymmetry()
        {
            foreach (PAC.Patterns.Gradient.Radial gradient in testCases)
            {
                ...
                foreach (IntVector2 point in testRegion)
                {
                    Assert.True(gradient[point] == gradient[gradient.centre.coord + (point - gradient.centre.coord).Reflect(Axes.Vertical)], $"Failed with {gradient} and {point}.");
                    Assert.True(gradient[point] == gradient[gradient.centre.coord + (point - gradient.centre.coord).Reflect(Axes.Horizontal)], $"Failed with {gradient} and {point}.");
                }
            }
        }
